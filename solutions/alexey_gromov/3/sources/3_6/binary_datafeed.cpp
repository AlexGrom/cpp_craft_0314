...
int main(int argc, char* argv[])
{
	...
	map<string, ofstream*> outputFiles;

    while (!inputFile.eof())
    {
		binary_reader::stock_data data(inputFile);

		const string stock_name_str(data.stock_name());
 
		ofstream *outputFile = NULL;
	
 		if( outputFiles.count(stock_name_str) != 0 )
 		{
			outputFile = outputFiles[ stock_name_str ];
 		}
		else
		{
			outputFile = new ofstream(BINARY_DIR "/output_" + stock_name_str + ".txt", ios::out | ios::binary );
 			outputFiles[ stock_name_str ] = outputFile;
		}
